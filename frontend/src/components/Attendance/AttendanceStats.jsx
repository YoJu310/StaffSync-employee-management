    ...
    <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8'>
      {stats.map((s) => (
        <div key={s.label} className='card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group'>
            <div className="absolute left-0 top-0 bottom-0 w-1 rounded-r-full bg-slate-500/70 group-hover:bg-emerald-500/70"/>
            <div className='p-3 bg-slate-100 rounded-lg group-hover:bg-emerald-50 transition-colors duration-200'>
                <s.icon className="w-5 h-5 text-slate-600 group-hover:text-emerald-600 transition-colors duration-200"/>
            </div>
            <div>
