    ...
    const sidebarContent = (
        <>
            ...
            <div className='px-10 pt-10 pb-7'>
                <p className='text-[14px] font-semibold uppercase tracking-[0.15] text-slate-500 text-center'>Navigations</p>
            </div>
            ...
            <div className='flex-1 px-3 space-y-0.5 overflow-y-auto'>
                {loading ? (
                    ...
                    return (
                        <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-emerald-500/12 text-emerald-300" : "text-slate-300 hover-text-white hover:bg-white/4"}`}>
                            {isActive && <div className='absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-emerald-500' />}
                            <item.icon className={`w-[17px] h-[17px] shrink-0 ${isActive ? "text-emerald-300" : "text-slate-400 group-hover:text-slate-300"}`} />
                            <span className="flex-1">{item.name}</span>
                            {isActive && <ChevronRightIcon className="w-3.5 h-3.5 text-emerald-500/50" />}
                        </Link>
                    )
                })
                )}
